...
class MSFAHybridTaskCascadeRoIHead(CascadeRoIHead):
    ...
    def __init__(self,
                 ...
                 mask_info_flow=True,
                 **kwargs):
        super(MSFAHybridTaskCascadeRoIHead,
              self).__init__(num_stages, stage_loss_weights, **kwargs)
        self.feat_adapt = build_head(feat_adapt)
    ...
    def init_weights(self, pretrained):
        ...
        super(MSFAHybridTaskCascadeRoIHead, self).init_weights(pretrained)
    ...
    def with_mask_iou(self):
        """bool: whether the head has mask iou head"""
        if hasattr(self, 'mask_iou_head') and self.mask_iou_head is not None:
            return True
        else:
            return False
#
    ...
    def _slice_pos_features(self, adapt_feats, sampling_results):
    ...
    def _mask_forward_train(self,
        ...
        mask_feats = mask_feats + adapt_feats
        ...
        mask_results = dict(loss_mask=loss_mask, mask_feats=mask_feats,
                            mask_pred=mask_pred, mask_targets=mask_targets)
        return mask_results
    ...
    def _bbox_forward(self, stage, x, rois, semantic_feat=None):
        ...
        cls_score, bbox_pred, adapt_feats = bbox_head(bbox_feats, get_feature=True)

        bbox_results = dict(cls_score=cls_score, bbox_pred=bbox_pred,
                            bbox_feats=bbox_feats, adapt_feats=adapt_feats)
        return bbox_results
    ...
    def forward_train(self,
        ...
        for i in range(self.num_stages):
            ...
            adapt_feats = self._slice_pos_features(bbox_results['adapt_feats'],
                                                   sampling_results)
            adapt_feats = self.feat_adapt(adapt_feats)
            ...
            if i == self.num_stages - 1 and self.with_mask_iou:
                ...
                mask_iou_targets = self.mask_iou_head.get_targets(
                    ...
                    mask_results['mask_targets'], rcnn_train_cfg)
                loss_mask_iou = self.mask_iou_head.loss(pos_mask_iou_pred,
                                                        mask_iou_targets)
                losses['loss_mask_iou'] = loss_mask_iou['loss_mask_iou'] * sum(self.stage_loss_weights)
    ...
    def simple_test(self, x, proposal_list, img_metas, rescale=False):
        ...
        if self.with_mask:
            if det_bboxes.shape[0] == 0:
                ...
                # get bbox_feats and adapt_feats from final bboxes
                bbox_roi_extractor = self.bbox_roi_extractor[-1]
                bbox_feats = bbox_roi_extractor(
                    x[:len(bbox_roi_extractor.featmap_strides)], mask_rois)
                if self.with_semantic:
                    ...
                    if bbox_semantic_feat.shape[-2:] != bbox_feats.shape[-2:]:
                        bbox_semantic_feat = F.adaptive_avg_pool2d(
                            bbox_semantic_feat, bbox_feats.shape[-2:])
                    bbox_feats += bbox_semantic_feat
                _, _, adapt_feats = self.bbox_head[-1](
                    bbox_feats, get_feature=True)
                adapt_feats = self.feat_adapt(adapt_feats)
                ...
                if self.with_mask_iou:
                    mask_iou_pred = self.mask_iou_head(
                        mask_feats,
                        mask_pred[range(det_labels.size(0)), det_labels])
    ...
    def aug_test(self, img_feats, proposal_list, img_metas, rescale=False):
        ...
        if self.with_mask:
            if det_bboxes.shape[0] == 0:
                    ...
                    for i in range(self.num_stages):
                        ...
                        if self.with_mask_iou:
                            mask_iou_pred = self.mask_iou_head(
                                mask_feats,
                                mask_pred[range(det_labels.size(0)), det_labels])
                            aug_mask_ious.append(mask_iou_pred)
                ...
                if self.with_mask_iou:
                    merged_mask_ious = torch.stack(aug_mask_ious).mean(dim=0)
                    mask_scores = self.mask_iou_head.get_mask_scores(
                        merged_mask_ious, det_bboxes, det_labels)
